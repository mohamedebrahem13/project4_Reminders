...
in data package we are creating FakeDataSource  that acts as a test double to the LocalDataSource
 ...
 fun from ReminderDataSource in savereminder fun () we add reminder to the reminders list and it's suspend fun so we must call it from coroutines scope 
  ...
  in sandbarError() fun we are fakeDataSource.setReturnError(true) Make the repository return errors so the snack is not null THEN - It's an error, there's a snackBar we are cheking snackbar live data here so it's not null value  and  we are using assertThat to do that check  using hamcrest is for more readability
